...
echo ""
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "  I. Markdown 翻译覆盖率检查"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"

# 运行翻译覆盖率检查脚本
if [ -f "tests/e2e/check-markdown-translation-coverage.py" ]; then
    if python3 tests/e2e/check-markdown-translation-coverage.py > /tmp/coverage_check.log 2>&1; then
        check_pass "Markdown 翻译覆盖率达标"
    else
        check_fail "部分 Markdown 文件翻译覆盖率不足"
        echo ""
        echo "详情:"
        tail -20 /tmp/coverage_check.log | sed 's/^/   /'
    fi
else
    check_info "翻译覆盖率检查脚本不存在，跳过"
fi

echo ""
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "  J. YAML Unicode 检查"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"

# 运行 YAML Unicode 检查脚本
if [ -f "tests/e2e/check-yaml-unicode.py" ]; then
    if python3 tests/e2e/check-yaml-unicode.py > /tmp/yaml_check.log 2>&1; then
        check_pass "YAML Unicode 配置正确"
    else
        check_fail "发现 YAML dump 调用缺少 allow_unicode=True"
        echo ""
        echo "详情:"
        tail -15 /tmp/yaml_check.log | sed 's/^/   /'
    fi
else
    check_info "YAML Unicode 检查脚本不存在，跳过"
fi
...
echo ""
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "  M. CLI Help 本地化检查"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"

# 运行 CLI Help 本地化检查脚本
if [ -f "tests/e2e/check-cli-help-localization.py" ]; then
    if python3 tests/e2e/check-cli-help-localization.py > /tmp/cli_help_check.log 2>&1; then
        check_pass "CLI Help 已完全本地化"
    else
        check_fail "CLI Help 存在未翻译内容"
        echo ""
        echo "详情:"
        tail -20 /tmp/cli_help_check.log | sed 's/^/   /'
    fi
else
    check_info "CLI Help 本地化检查脚本不存在，跳过"
fi
